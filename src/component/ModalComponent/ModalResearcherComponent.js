...
class ModalResearcherComponent extends Component {
  constructor() {
    super();
    this.state = {
      open:false,
  ...
  Onsubmit = async () => {
    const { material_array, nutrient_array, product_topic } = this.state;
    const object = {
      product_id: this.props.product_id,
      product_topic: product_topic,
      nutrient_precent: JSON.stringify(nutrient_array),
      plant: JSON.stringify(material_array),
      image: this.state.imagePreviweUrl
    };
    // console.log("object", object);
    if (nutrient_array && material_array && product_topic) {
      try {
        await post(object, "plan/researcher_add_product_plan", user_token).then(
          res => {
            console.log(res);
            if (res.success) {
          ...
          <Row>
            <Col md={{ size: 10, offset: 1 }}>
              <Row>
                <Col md="12">
                  <FormGroup>
                    <Label for="product_topic">ชื่อสูตรผลิตภัณฑ์</Label>
                    <Input
                      style={{ fontSize: 20 }}
                      name="product_topic"
                      type="text"
              ...
              <Row>
                {this.state.material_array.length > 0 ? (
                  <Col>
                    <Card body>
...
export default ModalResearcherComponent;
